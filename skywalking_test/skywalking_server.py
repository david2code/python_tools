...
config.init(
    ...
    agent_name='python-server',                     # 服务名
    agent_instance_name='instance-server',                   # 实例名
)
...
parse.add_option(
    ...
    default="identify",
    dest="mock",
    help="run mock: identify, rewrite, weak, big_body, http_server, sendfile",
)
...
parse.add_option(
    "-s", "--piece_size", default="1", dest="piece_size", help="piece size"
)
parse.add_option(
    "-r", "--recive_file", default="", dest="receive_file", help="receive file path"
)
...
file_name = options.receive_file
print(file_name)
...
print(
    "run on: %s, filter: %s, content_type: %s, content_encoding: %s"
    % (host, mock, content_type, content_encoding)
)
...
class Resquest(BaseHTTPRequestHandler):
    ...
    def copyfile(self, source, outputfile):
        ...
        try:
            ...
            super().copyfile(
                source, outputfile
            )  # 调用父类方法（BaseHTTPRequestHandler 有 copyfile
    ...
    def request_process(self):
        ...
        if mock == "big_body":
            ...
            self.send_header(
                "set-cookie",
                "lang=zh-cn; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
            )  # 设置服务器响应头
            self.send_header(
            ...
            if login_token_position == "header":
                self.send_header(
                    "token",
                    "eyj0exaioijkv1qilcjhbgcioijiuzi1nij9.eyjsyxqioje3mdq2otk0mdqsim5izii6mtcwndy5otqwncwizxhwijoxnza0nzg1oda0lcj1awqioii1ntailcj0exblijoidxnlciisinjhbmqioiixnza0njk5nda0dmnrztbwcgeifq.tpebdwaih2hemlnjeoytrjplufsdqfd6xnuq-f-6wpi",
                )  # 设置服务器响应头
                if login_success == "0":
                ...
                if login_success == "0":
                    ...
                    self.send_header(
                        "set-cookie",
                        "msg=nok; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
                    )
                else:
                    print("set login success: msg=ok")
                    self.send_header(
                        "set-cookie",
                        "msg=ok; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
                    )
                ...
                self.send_header(
                    "set-cookie",
                    "username=davidlss; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
                )  # 设置服务器响应头
                self.send_header(
                    "set-cookie",
                    "password=davidlss; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
                )  # 设置服务器响应头
                self.send_header(
                    "set-cookie",
                    "emal=davidlss@126.com; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
                )  # 设置服务器响应头
                self.send_header(
                    "set-cookie",
                    "token=eyj0exaioijkv1qilcjhbgcioijiuzi1nij9.eyjsyxqioje3mdq2otk0mdqsim5izii6mtcwndy5otqwncwizxhwijoxnza0nzg1oda0lcj1awqioii1ntailcj0exblijoidxnlciisinjhbmqioiixnza0njk5nda0dmnrztbwcgeifq.tpebdwaih2hemlnjeoytrjplufsdqfd6xnuq-f-6wpi; expires=wed, 07-feb-2024 07:27:44 gmt; max-age=2592000; path=/zentao/",
                )  # 设置服务器响应头
            ...
            self.send_header(
                "set-cookie",
                "rememberMe=deleteMe; Path=/; Max-Age=0; Expires=Tue, 16-Apr-2024 09:18:14 GMT; SameSite=lax",
            )
            self.send_header(
                "set-cookie",
                "rememberMe=deleteMe; Path=/; Max-Age=0; Expires=Tue, 16-Apr-2024 09:18:14 GMT; SameSite=lax",
    ...
    def deal_upload(self):
        ...
        if file_name == "":
            ...
            if (
                ...
                and self.headers["Transfer-Encoding"].lower() == "chunked"
            ):
            ...
            file_name = self.headers.get("filename", "uploaded_file")
            ...
            if (
                ...
                CHUNK_SIZE = 16384  # 每次读取16KB
                try:
                    with open(file_name, "wb") as f:
                        remaining = int(self.headers["content-length"], 0)
                        while remaining > 0:
                            # 读取不超过剩余大小的数据块
                    ...
                    self.end_headers()
                    self.wfile.write(f"服务器内部错误: {e}".encode("utf-8"))
